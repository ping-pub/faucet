  ...
  try {
    ...
    if (chainConf) {
      if (chainConf.type === 'Ethermint') {
        ...
        const faucetAddress = toBech32(chainConf.sender.option.prefix, fromHex(wallet2.address.replace('0x', '')));
        await client.getBalance(faucetAddress, chainConf.tx.amount.denom).then(x => {
          return balance = x
